    ...
    <nav class="flex" aria-label="Breadcrumb">
        <ol class="inline-flex items-center lg:my-4 my-2 space-x-1 md:space-x-3">
            ...
            <li>
                <div class="flex items-center">
                <svg aria-hidden="true" class="w-6 h-6 text-slate-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
                <a href="{{ url('/transaksi_masuk/'.$data->kode_transaksi) }}" class="ml-1 text-sm font-medium text-slate-700 hover:text-red-700 md:ml-2">{{ $subtitle }}</a>
                </div>
            </li>
        </ol>
    </nav>
...
<div class='bg-white rounded-lg shadow-md mt-5'>
    ...
    <div class='p-5'>
        ...
        <div class="grid grid-cols-12 gap-1 border-b border-slate-300 lg:py-3 py-2">
            ...
            <div class="lg:col-span-9 col-span-12 my-auto font-light text-slate-600">{{ $data->nama_customer }}</div>
        </div>
        ...
        <div class="grid grid-cols-12 gap-1 border-b border-slate-300 lg:py-3 py-2">
            <div class="lg:col-span-3 col-span-12 font-semibold text-slate-600">Tujuan Pembayaran</div>
            <div class="lg:col-span-9 col-span-12 my-auto font-light text-slate-600">{{ $data->tujuan_pembayaran }}</div>
        </div>

        <div class="grid grid-cols-12 gap-1 border-b border-slate-300 lg:py-3 py-2">
            <div class="lg:col-span-3 col-span-12 font-semibold text-slate-600">Nominal</div>
            <div class="lg:col-span-9 col-span-12 my-auto font-light text-slate-600">Rp {{ number_format($data->nominal, 0, ',', '.') }}</div>
        </div>

        <div class="grid grid-cols-12 gap-1 border-b border-slate-300 lg:py-3 py-2">
            <div class="lg:col-span-3 col-span-12 font-semibold text-slate-600">Keterangan</div>
        ...
        <div class="mt-8 mb-4 flex">
            <a href="{{ url('/transaksi_masuk/'.$data->kode_transaksi.'/edit') }}" name='edit' class='flex mr-2 px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700 focus:ring focus:ring-emerald-200'>
            ...
            <a href='/transaksi_masuk' name='back' class='px-3 py-2 rounded-lg bg-slate-500 text-white hover:bg-slate-700 focus:ring focus:ring-slate-300'>Kembali</a>
        </div>
    </div>
</div>
@endsection