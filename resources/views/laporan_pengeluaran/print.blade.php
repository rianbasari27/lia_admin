...
<head>
    ...
    <title>Laporan Kas Keluar - {{ $bulan[$month_now] }} {{ $year_now }}</title>
</head>
<body class="mx-5">
            ...
            <table class="w-full lg:text-sm text-[8px]">
                <thead>
                    <tr>
                        ...
                        <th class="py-2 border border-slate-700">Jumlah Supplier</th>
                        <th class="py-2 border border-slate-700">Jumlah Transaksi</th>
                        <th class="py-2 border border-slate-700">Uang Keluar</th>
                    </tr>
                </thead>
                @if ($data->count())
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="px-2 border border-slate-700 text-end">{{ $item->jumlah_supplier }}</td>
                            ...
                            <td class="px-2 border border-slate-700 text-end">
                                <div class="justify-between">
                                    Rp {{ ($item->uang_keluar == null) ? '-' : number_format($item->uang_keluar, 0, ',', '.') }}
                                </div>
                            </td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr>
                            <td colspan="5" class="border border-slate-700 px-2 text-end font-semibold">Total Kas Keluar</td>
