    ...
    <aside id="logo-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen pt-20 transition-transform -translate-x-full bg-white border-r border-slate-200 sm:translate-x-0 dark:bg-slate-800 dark:border-slate-700" aria-label="Sidebar">
        <div class="h-full px-3 pb-4 overflow-y-auto bg-white dark:bg-slate-800">
            <ul class="space-y-2 font-medium mx-2">
                ...
                <li>
                    <button type="button" class="flex {{ $title == 'Laporan Pemasukan' || $title == 'Laporan Pengeluaran' ? 'bg-red-700 text-white font-semibold' : 'hover:bg-slate-100' }} items-center w-full p-2 text-slate-500 transition duration-75 rounded-lg group" aria-controls="dropdown-laporan" data-collapse-toggle="dropdown-laporan">
                        <i class="fa-solid fa-clipboard text-lg ml-1"></i>
                        <span class="flex-1 ml-4 text-left whitespace-nowrap">Laporan</span>
                        <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                    </button>
                    <ul id="dropdown-laporan" class="{{ $title == 'Laporan Pemasukan' || $title == 'Transaksi Pengeluaran' ? '' : 'hidden' }} py-2 space-y-2">
                        <li>
                            <a href="/laporan_pemasukan" class="flex {{ $title == 'Laporan Pemasukan' ? 'bg-red-300 text-white font-semibold' : 'hover:bg-slate-100' }} items-center w-full p-2 text-slate-500 transition duration-75 rounded-lg pl-11 group">Laporan pemasukan</a>
                        </li>
                        <li>
                            <a href="/laporan_pengeluaran" class="flex {{ $title == 'Laporan Pengeluaran' ? 'bg-red-300 text-white font-semibold' : 'hover:bg-slate-100' }} items-center w-full p-2 text-slate-500 transition duration-75 rounded-lg pl-11 group">Laporan pengeluaran</a>
                        </li>
                    </ul>
                </li>
                <li>
